...
pub fn create_player(name: &str) -> Result<Player> {
    ...
    diesel::insert(&new_player).into(player::table).get_result(&conn)
}
...
pub fn create_category(text: &str) -> Result<Category> {
    ...
    let conn = establish_connection();
    diesel::insert(&new_category).into(category::table).get_result(&conn)
}

pub fn get_categories() -> Result<Vec<Category>> {
    use self::schema::category::dsl::*;
    let conn = establish_connection();
    category.filter(is_active.eq(true)).load::<Category>(&conn)
}
